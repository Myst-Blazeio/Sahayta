...
import { Shield, Clock, MapPin, Navigation, AlertTriangle, AlertCircle, Zap, Car, Bike, Footprints } from 'lucide-react';
...
const SafeRouteTab: React.FC = () => {
    ...
    const [transportMode, setTransportMode] = useState<'car' | 'bike' | 'cycling' | 'walking'>('car');

    // Results
    const [stats, setStats] = useState<RouteStats | null>(null);
    const [mapUrl, setMapUrl] = useState<string>(`http://localhost:5000/api/safe-route/map?start_lat=22.5726&start_lng=88.3639&end_lat=22.5726&end_lng=88.3639&type=${routeType}&mode=${transportMode}&_t=${Date.now()}`);
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (startRef.current && !startRef.current.contains(event.target as Node)) setShowStartSugg(false);
            if (endRef.current && !endRef.current.contains(event.target as Node)) setShowEndSugg(false);
        };
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            if (startText.length > 2 && showStartSugg) {
                try {
                    const res = await fetch(`http://localhost:5000/api/safe-route/autocomplete?q=${encodeURIComponent(startText)}`);
    ...
    useEffect(() => {
        const timer = setTimeout(async () => {
            if (endText.length > 2 && showEndSugg) {
                try {
                    const res = await fetch(`http://localhost:5000/api/safe-route/autocomplete?q=${encodeURIComponent(endText)}`);
    ...
    const findRoute = async () => {
        ...
        try {
            ...
            if (!startPos) {
                const startRes = await fetch(`http://localhost:5000/api/safe-route/geocode?q=${encodeURIComponent(startText)}`);
                if (!startRes.ok) throw new Error(`Could not find location: ${startText}`);
            ...
            if (!endPos) {
                const endRes = await fetch(`http://localhost:5000/api/safe-route/geocode?q=${encodeURIComponent(endText)}`);
            ...
            setMapUrl(`http://localhost:5000/api/safe-route/map?start_lat=${finalStartLat}&start_lng=${finalStartLng}&end_lat=${finalEndLat}&end_lng=${finalEndLng}&type=${routeType}&bbox=${bbox}&mode=${transportMode}&_t=${Date.now()}`);

            // Fetch dynamic stats
            const url = `http://localhost:5000/api/safe-route/?start_lat=${finalStartLat}&start_lng=${finalStartLng}&end_lat=${finalEndLat}&end_lng=${finalEndLng}&type=${routeType}&mode=${transportMode}`;
    ...
    const handleGetCurrentLocation = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                setMapUrl(`http://localhost:5000/api/safe-route/map?start_lat=${latitude}&start_lng=${longitude}&end_lat=${endL}&end_lng=${endLn}&type=${routeType}&mode=${transportMode}&_t=${Date.now()}`);

                try {
                    // Reverse geocode to get a readable address
                    const res = await fetch(`http://localhost:5000/api/safe-route/reverse-geocode?lat=${latitude}&lng=${longitude}`);
                    if (res.ok) {
                        const data = await res.json();
        ...
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-200px)] min-h-[600px]">
            ...
            <div className="w-full lg:w-1/3 bg-card border rounded-lg shadow-sm p-4 flex flex-col gap-5 official-card overflow-y-auto">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Dynamic pathfinding bypassing active crime hotspots.</p>
                </div>
                        ...
                        {showStartSugg && startSuggestions.length > 0 && (
                            <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-48 overflow-auto">
                                {startSuggestions.map((s, i) => (
                                    <li key={i} onClick={() => selectStart(s)} className="p-3 hover:bg-blue-50 cursor-pointer text-sm border-b last:border-0 flex items-center gap-2">
                        ...
                        {showEndSugg && endSuggestions.length > 0 && (
                            <ul className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-48 overflow-auto">
                                {endSuggestions.map((s, i) => (
                                    <li key={i} onClick={() => selectEnd(s)} className="p-3 hover:bg-blue-50 cursor-pointer text-sm border-b last:border-0 flex items-center gap-2">
                                        <MapPin size={14} className="text-gray-400 flex-shrink-0" />
                                        <span className="truncate">{s.address}</span>
                                    </li>
                                ))}
                    ...
                    <div className="flex-1 mt-2 border border-gray-100 rounded-xl bg-white shadow-sm p-5 flex flex-col justify-center animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-5 border-b pb-4">
                            <div className="flex flex-col gap-0.5">
                                <span className="text-xs text-gray-500 uppercase tracking-widest font-bold">ETA</span>
                                <span className="text-2xl font-black flex items-center gap-1.5 text-gray-900"><Clock size={20} className="text-blue-500" /> {formatDuration(stats.duration)}</span>
                            </div>
                            <div className="flex flex-col gap-0.5">
                                <span className="text-xs text-gray-500 uppercase tracking-widest font-bold">Distance</span>
                                <span className="text-2xl font-black flex items-center gap-1.5 text-gray-900"><Navigation size={20} className="text-indigo-400" /> {formatDistance(stats.distance)}</span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between mb-5">
                            <div className="flex items-center gap-2 text-sm font-semibold text-gray-600">
                                <AlertTriangle size={16} className={stats.high_risk_zones > 0 ? "text-red-500" : "text-green-500"} />
                                High-Risk Crossings
                            </div>
                            <div className={`font-bold text-sm ${stats.high_risk_zones > 0 ? "text-red-600" : "text-green-600"}`}>
                                {stats.high_risk_zones} zones
                            </div>
                        </div>

                        <div className="p-4 rounded-xl flex items-center justify-between" style={{
                            backgroundColor: stats.safety_score > 80 ? 'rgba(34, 197, 94, 0.1)' : (stats.safety_score > 60 ? 'rgba(234, 179, 8, 0.1)' : 'rgba(239, 68, 68, 0.1)'),
                            border: `1px solid ${stats.safety_score > 80 ? 'rgba(34, 197, 94, 0.3)' : (stats.safety_score > 60 ? 'rgba(234, 179, 8, 0.3)' : 'rgba(239, 68, 68, 0.3)')}`
                        }}>
                            <div>
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1">Exposure Level</span>
                                <span className={`font-black uppercase tracking-wide flex items-center gap-1.5 ${stats.risk_level === 'Low' ? 'text-green-600' : (stats.risk_level === 'Medium' ? 'text-yellow-600' : 'text-red-600')}`}>
                                    {stats.risk_level === 'Low' ? <Shield size={18} /> : <AlertTriangle size={18} />}
                                    {stats.risk_level}
                                </span>
                            </div>
                            <div className="text-right">
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-500 block mb-1">Safety Index</span>
                                <span className="text-4xl font-black tracking-tighter" style={{
                                    color: stats.safety_score > 80 ? '#16a34a' : (stats.safety_score > 60 ? '#ca8a04' : '#dc2626')
                                }}>
                                    {stats.safety_score}
            ...
            <div className="w-full lg:w-2/3 border rounded-xl overflow-hidden shadow-sm relative z-0 bg-gray-50">
                ...
                <div className="absolute bottom-6 right-6 bg-white/95 backdrop-blur-md p-4 rounded-xl shadow-xl border border-gray-100 z-[1000] text-sm pointer-events-none transition-all">
                    <h4 className="font-black mb-3 text-[10px] uppercase tracking-widest text-gray-500 border-b pb-2">Dynamic Threat Radar</h4>

                    {stats?.high_risk_zones !== undefined && stats.high_risk_zones > 3 && (
                    ...
                    <div className="flex items-center gap-3 mb-2 font-medium">
                        ...
                        <span className={stats?.risk_level === 'High' ? 'font-bold text-red-600' : 'text-gray-600'}>High Risk</span>
                    </div>
                    <div className="flex items-center gap-3 mb-2 font-medium">
                        <div className="w-3 h-3 rounded-full bg-[#ff7f50] shadow-[0_0_8px_rgba(255,127,80,0.5)]"></div>
                        <span className={stats?.risk_level === 'Medium' ? 'font-bold text-orange-600' : 'text-gray-600'}>Medium Risk</span>
                    </div>
                    <div className="flex items-center gap-3 font-medium">
                        <div className="w-3 h-3 rounded-full bg-blue-600 shadow-[0_0_8px_rgba(37,99,235,0.5)]"></div>
                        <span className={stats?.risk_level === 'Low' ? 'font-bold text-blue-600' : 'text-gray-600'}>Low Risk</span>
                    </div>
                </div>
            </div>
        </div>
